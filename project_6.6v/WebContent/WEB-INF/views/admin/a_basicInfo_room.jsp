...
<body>
				...
				<div class="container-fluid">
					<div class="navbar-wrapper">
						...
						<a class="navbar-brand"
							...
							></a> <a class="navbar-brand">기초 정보 관리 ></a> <a class="navbar-brand"
							href="">강의실 관리</a>
					</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header text-right">
								<div class="pull-left">
									<h4 class="card-title">강의실 관리</h4>
								</div>
								<button type="button" class="btn btn-default btn-icon btnDel"
									data-toggle="modal" data-target="#addRoom"title="추가">
											...
											<tr>
												...
												<td>
													<button type="button" class="btn btn-default btn-icon btnUpdate"
														data-toggle="modal" data-target="#updateRoom" title="수정" value="${r.room_id}">
														<i class="fa fa-pencil-square-o"></i>
													</button>
													<button type="button" class="btn btn-default btn-icon btnDel"
														data-target="#delRoom" data-toggle="modal" title="삭제" value="${r.room_id}"
														${r.deletable == 0 ? "":"disabled"}>
	...
	<div id="addRoom" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<form action="${pageContext.request.contextPath}/a/classroom/add" method="post">
					<div class="modal-body">
						<div class="form-group">
							<label>강의실명</label>
							<input type="text" class="form-control" name="room_name" placeholder="max 20" required>
						</div>

						<div class="form-group">
							<label>정원</label>
							<input type="number" class="form-control" name="room_capacity" placeholder="max 20" required>
						</div>
					</div>
	...
	<!-- 강의실 삭제 -->
	<div id="delRoom" class="modal fade" role="dialog">
		<div class="modal-dialog">

			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">강의실 삭제</h5>
				</div>
				<form action="${pageContext.request.contextPath}/a/classroom/del" method="post">
					<div class="modal-body">
	...
	<div id="updateRoom" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">강의실 수정</h5>
				</div>
				<form action="${pageContext.request.contextPath}/a/classroom/update" method="post">
					<div class="modal-body">
						<div class="form-group">
							<label>강의실번호</label>
							<input type="text" class="form-control room_id" name="room_id" readonly="readonly" required>
						</div>

						<div class="form-group">
							<label>강의실명</label>
							<input type="text" class="form-control room_name" name="room_name" required>
						</div>
						
						<div class="form-group">
							<label>정원</label>
							<input type="number" class="form-control room_capacity" name="room_capacity" required>
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">CANCEL</button>
	...
	<script>
		$(document).ready(function() {
			$(".btnUpdate").on("click", function(){
				...
				var room_name = $(this).parents("td").siblings("td").eq(1).text();
				var room_capacity = $(this).parents("td").siblings("td").eq(2).text();
				$("#updateRoom .room_id").val(room_id);
				$("#updateRoom .room_name").val(room_name);
				$("#updateRoom .room_capacity").val(room_capacity);
			});
			$(".btnDel").on("click", function(){
				var room_id = $(this).val();
				var room_name = $(this).parents("td").siblings("td").eq(1).text();
				$("#delRoom .room_id").val(room_id);
	      		$("#delRoom #info").text(room_id + " (" + room_name + ")");
			});
		});
	</script>
</body>
